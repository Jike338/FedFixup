...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model, _, optimizer = create_model(args)
    ...
    activation_single = {}
    activation_quantile = {}
    optimizer_glob_avg = optimizer.state_dict()
    ...
    for r in range(args.start_round, args.rounds, 1):
        ...
        for client_id in client_ids_round:
            ...
            if args.fix_warmup and r >= args.rounds/3:
                ...
                optimizer.load_state_dict(copy.deepcopy(optimizer_glob_avg))
            ...
            optimizer_locals.append(copy.deepcopy(optimizer.state_dict()))
        ...
        optimizer_glob_avg = FedAvg2(optimizer_locals, size_arr = participating_size)
...
def FedAvg2(w , global_w=None, size_arr=None):
    w_avg = {}
    w_avg['param_groups'] = w[0]['param_groups']
    w_avg['state'] = {}

    for k in w[0]['state'].keys():
    #   if "num_batches_tracked" in k: continue      
      w_avg['state'][k] = {}
      w_avg['state'][k]['momentum_buffer'] = torch.zeros(w[0]['state'][k]['momentum_buffer'].size())
    ...
    for k in w_avg['state'].keys():
    #   if "num_batches_tracked" in k: continue 
      for i in range(0, len(w)):
        w_avg['state'][k]['momentum_buffer'] += size_arr[i]*w[i]['state'][k]['momentum_buffer'].cpu()
      w_avg['state'][k]['momentum_buffer'] = torch.div(w_avg['state'][k]['momentum_buffer'], len(w))
