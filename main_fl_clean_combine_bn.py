...
parser.add_argument('--log_every_round', default=5, type =int)  
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for r in range(args.start_round, args.rounds, 1):
        ...
        for client_id in client_ids_round:
            ...
            model_freeze, _, _ = create_model(args)
            model_freeze.load_state_dict(copy.deepcopy(w_glob_avg))
            ...
            if args.steps > 0:
                ...
                train_with_step(train_loader, model, model_freeze, criterion, optimizer, 0, args, r)
            else:
            ...
            #     }, False, args.logdir, filename='checkpoint_r{}_client{}.pth.tar'.format(r, client_id))             
            # '''
        ...
        if args.freeze_bn_at_round > 0 and r % args.freeze_bn_at_round-1 == 0:
            save_checkpoint({
                'epoch': r + 1,
                'arch': args.arch,
                'state_dict': model.state_dict(),
                'best_acc1': best_acc1,
                'optimizer' : optimizer.state_dict(),
            }, is_best, args.id.split("/")[0]+"/models", filename='%s_checkpoint_r%d.pth.tar'%(args.id.split("/")[2],r))
...
def train_with_step(train_loader, model, model_freeze, criterion, optimizer, epoch, args, r):
    batch_time = AverageMeter('Time', ':6.3f')
    data_time = AverageMeter('Data', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    ...
    while total_step <= args.steps:
        ...
        # compute output
        output = model(images)
        output_freeze = model_freeze(images)

        loss = criterion(output, target)
        loss_freeze = criterion(output_freeze, target)

        # if args.freeze_bn_at_round > 0 and r >=args.freeze_bn_at_round and args.update_bn_after_freeze:
        loss = loss + 1/2*(torch.cdist(loss, loss_freeze, p=2.0)
