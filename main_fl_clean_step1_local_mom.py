...
parser.add_argument('--bn_step', default=0, type=int)
parser.add_argument('--update_bn_before', action='store_true')
parser.add_argument('--update_bn_after', action='store_true')
parser.add_argument('--update_bn_w_mem', action='store_true')
parser.add_argument('--exp', default=0, type=int)
parser.add_argument('--eval_every_round', default=5, type=int)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Data loading code
    client_train_loaders, val_loader, test_loader, cnts_dict = load_client_data(args)    
    ...
    for r in range(args.start_round, args.rounds, 1):
        ...
        for client_id in client_ids_round:
            ...
            train_loader_cp = client_train_loaders[client_id]
            try:
                # Samples the batch
            ...
            if args.fix_warmup and r >= args.rounds/3:
                ...
                model, criterion, optimizer = create_model(args)
                if client_id in local_mem.keys():
                    optimizer.load_state_dict(copy.deepcopy(local_mem[client_id]))
            ...
            if args.update_bn_before and args.bn_step > 0:
                if args.update_bn_w_mem:
                    update_bn_with_step(train_loader_cp, model, args)
                else:
                    update_bn_with_step_wo_mem(train_loader_cp, model, args)

            if args.steps > 0:
                adjust_learning_rate_with_step(optimizer, r, args)
                train_with_step_minibatch(minibatch, model, criterion, optimizer, 0, args, r)
            else:
                adjust_learning_rate(optimizer, r, args)
                for epoch in range(args.local_epochs):
                    train(train_loader, model, criterion, optimizer, epoch, args, r)

            if args.update_bn_after and args.bn_step > 0:
                if args.update_bn_w_mem:
                    update_bn_with_step(train_loader_cp, model, args)
                else:
                    update_bn_with_step_wo_mem(train_loader_cp, model, args)
            ...
            local_mem[client_id] = optimizer.state_dict()
...
def load_client_data(args):
    ...
    return client_train_loader_dict, val_loader, test_loader, cnts_dict 
...
def create_model(args):
    ...
    if "mobilenetv1" in args.arch:
        model = mobilenetv1.mobilenet_v1(num_classes=args.num_classes, norm_layer=args.norm_layer)
